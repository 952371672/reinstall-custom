...
set confhome=https://raw.githubusercontent.com/952371672/reinstall-custom/main
rem 国内机器优先使用 GitHub raw；如果访问不稳定，再把下面地址换成你的国内镜像 raw 地址
set confhome_cn=https://cnb.cool/952371672/reinstall-custom/-/git/raw/main
...
rem 国内机器固定按中国网络处理，跳过 geoip 探测，避免 Qualcomm/Cloudflare 请求卡住
rem 如果要把脚本用于海外机器，把 set ForceCN=1 改为 set ForceCN=0
set ForceCN=1
if "!ForceCN!"=="1" (
    >geoip echo loc=CN
) else (
    rem 海外模式才进行 geoip 探测；国内模式不会访问这些地址
    if not exist geoip (
        call :download https://www.qualcomm.cn/cdn-cgi/trace %~dp0geoip || call :download https://www.cloudflare.com/cdn-cgi/trace %~dp0geoip || goto :download_failed
    )
    findstr /c:"loc=" geoip >nul || (echo Invalid geoip file & del /q geoip 2>nul & exit /b 1)
)
...
if not errorlevel 1 (
    ...
    rem 国内优先使用南京大学 HTTPS 镜像；如果该镜像不可用可改为 https://mirrors.ustc.edu.cn/cygwin 或 https://mirrors.tuna.tsinghua.edu.cn/cygwin
    set mirror=https://mirror.nju.edu.cn
...
echo [1/6] Checking Cygwin...
call :check_cygwin_installed || (
    echo [2/6] Cygwin not installed, preparing installer...
    ...
    echo [3/6] Downloading Cygwin installer...
    if not exist setup-!CygwinArch!.exe (
        call :download https://files.m.daocloud.io/www.cygwin.com/setup-!CygwinArch!.exe %~dp0setup-!CygwinArch!.exe || call :download https://www.cygwin.com/setup-!CygwinArch!.exe %~dp0setup-!CygwinArch!.exe || goto :download_failed
    )
    ...
    rem 安装 Cygwin；使用 quiet-mode 全自动安装，不需要手动点击下一步
    set site=!mirror!!dir!
    echo [4/6] Installing Cygwin automatically from: !site!
    start "Cygwin Setup" /wait setup-!CygwinArch!.exe ^
...
rem 下载 reinstall.sh；每次启动前将 Alpine 的国内源设为阿里云，避免 mirror.nju.edu.cn DNS transient error
...
rem DD 模式会在 Alpine 环境中通过 apk 下载 libaio 等包；固定使用阿里云 HTTP 源
%SystemDrive%\cygwin\bin\sed -i "s#http://mirror.nju.edu.cn/alpine#http://mirrors.aliyun.com/alpine#g" %thisdir%reinstall.sh
rem 清理可能已缓存的 Alpine 源变量，强制重新按修改后的脚本生成参数
...
rem 国内机器必须把 --force-cn 传给 reinstall.sh；前面的 geoip 只影响本 bat，不能影响 bash 子脚本
%SystemDrive%\cygwin\bin\bash %thisdir%reinstall.sh --force-cn %*
exit /b
