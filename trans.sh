...
# DD 安全保活：磁盘镜像写入成功后，立即把 FRP 写入目标系统并设置开机启动。
# 这一步独立于后续的 modify_os_on_disk；后续步骤失败时目标系统仍可通过 FRP 回连。
install_frpc_early_on_disk() {
    if ! ls /configs/frpc.* >/dev/null 2>&1; then
        return 0
    fi

    info "Install emergency persistent frpc"
    update_part || {
        warn "Cannot refresh partition table; temporary frpc will remain available."
        return 0
    }
    ...
    for part in $(lsblk /dev/$xda --filter 'TYPE == "part"' --sort SIZE -no NAME | tac); do
        ...
        if etc_dir=$({ ls -d /os/etc/ || ls -d /os/*/etc/; } 2>/dev/null); then
            ...
            umount /os || true
            if ! mount -o rw /dev/$part /os; then
                warn "Cannot remount $part read-write; trying the next partition."
                continue
            fi
            if [ -d "$os_dir/etc/systemd" ]; then
                add_frpc_systemd_service_if_need "$os_dir" ||
                    warn "Persistent systemd frpc setup failed; temporary frpc remains available."
            else
                warn "Target OS is not systemd; persistent early FRP setup skipped."
            fi
            sync
            umount /os || true
            return 0
    ...
    warn "Target OS partition was not found; temporary frpc remains available."
    return 0
}
...
install_redhat_ubuntu() {
    ...
    if [ "$distro" = "ubuntu" ]; then
        ...
        # 保留 FRP 配置到 Ubuntu 安装器可访问的安装分区，供 late-commands 提前落盘
        if ls /configs/frpc.* >/dev/null 2>&1; then
            cp -f /configs/frpc.* /os/frpc.conf
        fi
        mkdir -p /iso
...
# 设置临时 frpc
# 先确认 FRP 至少成功登录服务器，再继续后面的重装流程。
# 这一段发生在正式分区/写盘之前；如果临时 FRP 在限定时间内无法连通，
# 直接重启。上游通过一次性启动项进入 Alpine，因此通常会回到原系统。
if ls /configs/frpc.* >/dev/null 2>&1 && ! pidof frpc >/dev/null; then
    info 'run frpc'
    add_community_repo
    apk add frp

    frpc_tmp_log=/run/frpc-init.log
    frpc_tmp_pid=
    frpc_tmp_connected=0
    frpc_tmp_wait=90
    : >"$frpc_tmp_log"

    frpc -c /configs/frpc.* >>"$frpc_tmp_log" 2>&1 &
    frpc_tmp_pid=$!

    info "wait temporary frp connection (${frpc_tmp_wait}s)"
    for _ in $(seq 1 "$frpc_tmp_wait"); do
        if grep -Eiq 'login to server success|start proxy success|start visitor success|connected to server' "$frpc_tmp_log"; then
            frpc_tmp_connected=1
            break
        fi
        if ! kill -0 "$frpc_tmp_pid" 2>/dev/null; then
            break
        fi
        sleep 1
    done

    if [ "$frpc_tmp_connected" != 1 ]; then
        warn "temporary FRP did not connect; abort installation and reboot to the original system"
        sed -n '1,80p' "$frpc_tmp_log" >&2 || true
        sync
        reboot
        sleep 30
        error_and_exit "reboot did not return to the original system"
    fi

    info 'temporary frp connected; continue installation'
    (
        while true; do
            if ! kill -0 "$frpc_tmp_pid" 2>/dev/null; then
                frpc -c /configs/frpc.* >>"$frpc_tmp_log" 2>&1 &
